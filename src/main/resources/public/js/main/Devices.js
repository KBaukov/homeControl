Ext.define('Devices', {
    ...
    delDevice: function(grid, rowIndex) {
        ...
        Ext.Msg.show({
            ...
            fn: function(btn){
                if(btn == "yes") {
                    Ext.Ajax.request({
                        url: '/api/device/delete', scope: this, method: 'GET',
                        params: {id: id},
    ...
    saveData: function() { 
        ...
        if(n>0) {
            for(var i=0; i<n; i++) {
                var data = {id: modify[i].data.id, type: modify[i].data.type, name:modify[i].data.name, ip:modify[i].data.ip, active_flag: modify[i].data.active_flag, description:modify[i].data.description };
                Ext.Ajax.request({
                    url: '/api/device/edit', scope: this, method: 'GET',
                    params: data,
                    success: function(response, opts) {
                      this.unmask();
                      var ansv = Ext.decode(response.responseText);
